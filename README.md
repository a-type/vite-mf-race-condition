...
- In the buggy path, production remote loader reads an entry from `__mf_module_cache__.share` and treats it as exports, but receives the wrong shape, causing a runtime error around `method`/`exportedThing`.
...
This usually makes the repro turn green (`OK:method-ok`) by forcing direct export objects into the cache.
